...
110 y=100:x=40
120 col=0:|COLAY,65,31,@col
130 |SETUPSP,31,9,43:|LOCATESP,31,y,x:|SETUPSP,31,0,65:' bolita
140 'ciclo de juego
150 |PRINTSP,31
170 IF INKEY(27) THEN 180
171 |MOVER,31,0,1:|COLAY,31,@col:IF col THEN |MOVER,31,0,-1:GOTO 190 ELSE GOTO 190
180 IF INKEY(34) THEN 190
181 |MOVER,31,0,-1:|COLAY,31,@col:IF col THEN |MOVER,31,0,1:GOTO 190 ELSE GOTO 190
190 IF INKEY(67) THEN 200
191 |MOVER,31,-2,0:|COLAY,31,@col:IF col THEN |MOVER,31,2,0:GOTO 210 ELSE GOTO 210
200 IF INKEY(69) THEN 210
201 |MOVER,31,2,0:|COLAY,31,@col:IF col THEN |MOVER,31,-2,0
210 GOTO 150

...
COP
		...
		srl e
		; ahora e =coordy/ 8
		ld H, 80; 80 en binario es 1010000
		...
		jp Z, COP_nosuma
		;---------------------------------------
		; a tiene las veces que hay q sumar 2048
		ld b,a; para el djnz lo necesito
		ld de,2048; un numero de 16 bit. no puedo usar mi rutina de muliplicar
		; aun asi es rapido porque como mucho es 7
COP_suma2048	add HL,de; se ejecuta 7 veces en el peor caso
		DJNZ COP_suma2048; un 1 seguido de 11 ceros
...
ANS_sigue		
		...
		;cp a,6; frames van de 0 a 6 como mucho. si estamos en 6 hay que ciclar
		cp a,7; frames van de 0 a 7 como mucho. si estamos en 6 hay que ciclar
...
;		INC HL
;		INC HL; ahora en HL esta la direccion donde se guarda la dir de la nueva imagen
...
SET_7		cp 7 
		...
		call ANS_a; invoco anima con el sprite id en regisro a
		ret
;============================================================================================================
...
_COLISION_SPRITES
		...
		ld a, NUM_SPRITES-1 ;empezamos por el ultimo sprite
		ld (CO2_sp2), a
		ld HL, 27496; direccion sprite numero 31 new
...
COS_bucle	
		...
		push HL;
		call CO2_j ;chequea la colision des sp1 con sp2
...
; variables locales
;-------------------------------------
SCS_pen		db 64; el byte (2 pix mode 0) que vamos a pintar
SCS_check	db 0
...
finbit		ld a,b
		...
		; recogemos el banco de estrellas
		;LD a, (ix+8)
		;ld (SCS_starbuffer), a
		;LD a, (ix+9)
		;ld (SCS_starbuffer+1), a
		ld a, (IX+8)
		and a
		JP NZ,SCS_bank1
SCS_bank0	LD HL, STARS0
	        ld (SCS_starbuffer),HL		
		JR SCS_bucle
SCS_bank1	
		LD HL, STARS1
		ld (SCS_starbuffer),HL		
...
;SCS_ini		ld hl,(SCS_starbuffer); inicializo HL a la direccion de las coord de estrellas
		;ld HL, STARS1; para pruebas!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
SCS_Ycycle	ld a, (SPR_MAX_Y); sale por ariba , entra por abajo
...
SCS_Xcycle	ld a, (SPR_MAX_X); sale por ariba , entra por abajo
		dec a; el max es justo el byte que ya no se pinta, debo restar 1
		jr SCS_finwritex
...
PS2:
		...
		ld a,(IX+0);menos signific. si es 0 no habra sync
		;cp a,0
		and A ; es como cp a,0 pero consume solo 1 ciclo.
...
; POKE dir, valor
;=============================================================================================================
; ejecuta comandos solicitados desde basic con POKE para evitar al analizador
; se invoca 300 veces por segundo
;
; variables locales
;-------------------------------------
_POKE

...
;|UMAP,dirmapini, dirmapfin,yini,yfin,xini,xfin
;
...
;--------------------------------------------------------------------------------------------------
; Ensambladas a partir de direccion 24500. La primera instruccion BASIC debe ser memory 24499 para
; colocar HIMEM en 24499 y asi no machacar las rutinas que comienzan en 24500
; los sprites deben dibujarse a partir de direccion 33600 por lo que disponemos de  8.4KB para graficos
; El programa basic podra ocupar hasta 24.5KB, pero cuidado, las variables de basic
; tambien ocupan memoria de modo que tu programa debe de ser menor de 24.5KB
; 
; salvar un programa creado con 8bp
; ---------------------------------
; para salvar las diferentes partes de tu programa ejecuta lo siguiente
;  SAVE "8BP.lib",b,24500,7700  version de 8BP especifica de tu juego. es necesario hacerla
;  SAVE "music.bin",b,32200,1300 tus musicas
;  SAVE "sprites.bin",b,33600,8440 tus sprites
;  SAVE "mapa.bin",b,42040,500 tu mapa
;
; para salvar tu juego mas facil puedes salvarlo todo de una sola vez
;  SAVE "todo.bin",b,24500,18040
;
...
m1PNTF_bin1	and a,%11110000; o bien %11001100
		JR Z,m1PNTF_fondo; todo ceros es para reestablecer el fondo en los 2 pixels transp o no transp
		; NOTA he observado que siembre salta a PTR_fondo porque pinto con colores que no usan los bits de fondo
		; pero si hubiese color 1, entonces se usaria para redondear el contorno de sprites
m1PNTF_bin2	cp %00001111; si los dos son 1, entonces no hacemos nada, no hay que reestablecer nada
		JR Z,m1PNTF_transp; 
...
m1PNTF_fondo	ld a, (DE); (DE) contiene el fondo "manchado" con restos del sprite en el frame anterior
m1PNTF_bin3	AND A,%11110000; bin %11100110 192 (192 es 128+64, es decir los dos 1). en total 230 192 , o bien  %11001100
		; el orden es 00221133	
		or a,c; OR del pixel con el fondo (fondo en "A" y pixel en "C")
...
_UMA
	...
	ld l, (IX+8)
	ld h, (IX+9)
	;ld (UMA_dirmap_fin),hl
	ld (UMA_dirmap_ptr),hl;* reverse orden .hl se debe igualar al dirmap_fin

	; recoge parametros
	ld l, (IX+10)
	ld h, (IX+11)
	ld (UMA_dirmap_ini),hl ; esta instruccion y la variable sobra
...
_INSTALL_RSX2
		...
		xor a; mode default de ordenamiento
		ld (SOR_mode),a
		;call _SORTY; se ordenan los sprites del 0 al 31
		;ret
		jP _SORTY ; optimize call +ret
...
PS2:
		...
		; estamos aqui por que hay un parametro
...
PS2_mas		ld a, (PS2_orden)
		...
		inc a ; optimizacion. es igual que ld a, 1 en este caso	
		ld (SOR_mode),a
...
;-------------------------------------------------
;_ROUTE_SP, spriteid; se puede invocar desde basic
;-------------------------------------------------
_ROUTE_SP
...
RSX_TABLE	defw RSX_NAMETB
		...
		JP _3D_RENDER3D		;|RENDER3D, spritefin, mode

...
; 26000	    MAP_TABLE tabla de hasta 82 entradas con el mapa del mundo en imagenes
;--------------------------------------------------------------------------------------------------
; Ensambladas a partir de direccion 25900. La primera instruccion BASIC debe ser memory 25899 para
; colocar HIMEM en 25899 y asi no machacar las rutinas que comienzan en 26000
...
; 42040 +---------- 
;       | sprites (hasta 8.5KB para dibujos. 8540 bytes si no hay secuencias ni rutas)
;       |
;       +---------- 
...
;       |		       
; 32250 +----------
;       |  rutinas 8BP  (5860 bytes, de los que sobran unos 75 bytes para mejoras futuras)
...
;================================================================================================================
; 
; comenzamos en 26000 con el mapa del mundo, que ocupa 390 bytes
; es necesario desde basic ejecutar memory 25999 al principio del programa
; el mapa se ha cargado en la direccion 26000 y puede almacenar hasta 64 elementos
;================================================================================================================
org 26390
...
;-----------------------------------------------------------------------------
; este es un trozo de la rutina _SETUP_SPRITE que no cabe en la zona de memoria principal
; y por eso la he ubicado aqui
SET10		cp 15
		RET NZ
...
ps2_chk3	and a
		JR z, PS2_all
		;call &bd19;_SYNC; sincronismo con barrido
		call _SYNC


		; pongo a cero el contador para controlar el tiempo de barrido por interrupciones. util si sync=2
		;ld a,0
		;ld (INS_counter2),a
...
PS2_bucle	
		;ld (SPR_ID), a;esto sobra. a no es un sprite id							***
		push HL
		;JR PS2_nosort
...
PS2_noc		
		...
		;-----mejora solapes. Esto hay que hacerlo si orden=true y sync=true
		;cp 15
		;call Z,&bd19
		;----------

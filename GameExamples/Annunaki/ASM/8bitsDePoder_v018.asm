...
CO2_j		ld a, (HL); leo status
		...
		; chequeo solape Y
		; -------------------
		ld bc,2; ayudita (2 lineas de margen)
		and a ; acarreo a cero
		...
		ld bc,1; ayudita (1 byte de margen)
		and a ; reset acarreo
		ld hl,(CO2x2ini)
		add HL,bc; ayudita para que no te coman facil ***********
		ld de, (CO2x1fin)
...
_COLISION_SPRITES
		...
		cp 32
		JR NZ,COS_OK
		; si sprite es 32 entonces es el comando de configuracion
		; la instruccion cp ocupa 1 byte
		ld a, (IX+0); valor del limite
		ld (COS_LIMIT+1),a
		ret
...
COS_sig		ld a, (CO2_sp2)
		dec a
		ld (CO2_sp2), a
COS_LIMIT	cp 0 ; esta comparacion se altera con |COLSP,32,valor

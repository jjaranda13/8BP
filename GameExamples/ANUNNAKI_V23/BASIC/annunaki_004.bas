...
750 if peek(27112)>0 then |colsp,7: if col<32 then dir=27113:goto 760
751 if peek(27128)>0 then |colsp,8: if col<32 then dir=27129:goto 760
752 if peek(27144)>0 then |colsp,9: if col<32 then dir=27145:goto 760
753 dc=dc mod 3 +1:dir=ddisp(dc):|peek,dir,@yd%: if yd%<-10 then poke dir-1,0: |POKE,dir,200:nd=nd-1
754 return: 
759'desactivo el disparo
760 poke dir-1,0: nd=nd-1:|PRINTSP,6,peek(dir),peek(dir+2): poke dir,255
761 if col>=duros then return
762 ' alcance de enemigo tipo blando. lo mismo q con el duro + pongo a muerte el blando
763 if col>=blandos then |SETUPSP,col,7,4:|SETUPSP,col,0,&x101:return 'secuencia de muerte sin colision
764 return
...
2192 |COLSP,31: if col%<32 then gosub 300:goto 2000
2193 gosub 750:'logica disparo, necesita tener hecha la colision.
2200 |AUTOALL
2201 |PRINTSPALL
2210 |STARS

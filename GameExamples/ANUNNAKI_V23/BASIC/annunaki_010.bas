...
86 avionimg=&9232:navemala7=&91ac
87 meteorito=&875a:borradisp=&8746:disparo=&8ac3:navemala1=&8ae6:
88 vidas =5:nd=0: bolaimg=&8e24:craterimg=&8618:navemala3=&8ed2
89 murol=&8e90:muror=&8e4e:navemala4=&8f58:navemala5=&8fde:NAVEMALA6=&9058
90 rocalr=&92ce:rocall=&93f4:rocarl=&9330:rocarr=&9392
91 minaimg=&9456: tierra=&94b8: navend=&95aa
99 |STARS,0,10,3,1,0  
...
780 if col%>=semis then ink 8,26:call &bd19:ink 8,10:semi(col)=semi(col)-1: if semi(col)=0 then |SETUPSP,col%,7,4:|SETUPSP,col%,0,&x101:return else return'secuencia de muerte sin colision
781 if col%>=blandos then |MOVER,col,2,1:|SETUPSP,col%,7,4:|SETUPSP,col%,0,&x101:return 'secuencia de muerte sin colision
785 return
...
3803 vy=1+(y-oy)/100
3810 |LOCATESP,bola,oy,ox:|SETUPSP,bola,5,vy,vx:|SETUPSP,bola,0,&x1011:|SETUPSP,bola,9,bolaimg:|SETUPSP,bola,7,0
3820 return
...
4600 for i=0 to 4:|SETUPSP,i,9,craterimg:|SETUPSP,i,0,&X1001:|SETUPSP,i,5,3,0
...
5010 |SETUPSP,i,9,navemala6:|SETUPSP,i,0,&x1011:|LOCATESP,i,-30*(i-19),38
...
6062 if ciclo mod 10=0 then avsp=avsp mod 5+1:|LOCATESP,avsp+9,-25,8+rnd*60:|SETUPSP,avsp+9,0,&X1011:|SETUPSP,avsp+9,9,avionimg:|SETUPSP,avsp+9,6,0: av2sp=(avsp +3) mod 5:avdir=27163+av2sp*16: flag=1:GOTO 6030
...
6200 for i=10 to 14:|SETUPSP,i,9,avionimg:|SETUPSP,i,0,&X1011:|SETUPSP,i,5,5,0:|LOCATESP,i,-1000,0:next
...
6556 if ciclo mod 24=0 then |SETUPSP,mina,0,&x1011:|SETUPSP,mina,5,3,0:|SETUPSP,mina,9,minaimg:|LOCATESP,mina,-20,(xl+xr)/2:mina=(mina+1)mod 4
...
6601 for i=10 to 30: |SETUPSP,i,5,3,0:|LOCATESP,i,200,0:|SETUPSP,i,0,&x1011:next
6602 for i=0 to 3:|SETUPSP,i,5,3,0:|LOCATESP,i,200,0:|SETUPSP,i,0,&x1011:next
...
7010 |SETUPSP,28,7,5: |SETUPSP,28,0,&x1111: |SETUPSP,28,5,0,0:'jefe1
7011 |SETUPSP,29,7,5: |SETUPSP,29,0,&x1111: |SETUPSP,29,5,0,0:'jefe2
7012 |SETUPSP,30,7,5: |SETUPSP,30,0,&x1111: |SETUPSP,30,5,0,0:'jefe3
...
8040 |SETUPSP,sp,0,&x101:|SETUPSP,sp,7,4: |LOCATESP,sp,ey,ex
...
9001 for i=10 to 24
9002 |SETUPSP,i,7,6:|SETUPSP,i,0,&x11111:|SETUPSP,i,5,1,0

...
88 vidas =3:nd=0: bolaimg=&8e24:craterimg=&8618:navemala3=&8ed2
89 murol=&8e90:muror=&8e4e:navemala4=&8f58:navemala5=&8fde
90 |STARS,0,10,3,1,0  
...
1114 if ciclo mod 6=0 then |peek,dirm,@ym% else 1117:rem mc-> meteorito counter
1115 if ym%>=200 then |poke,dirm,-30:|POKE,dirm+2,rnd*40+20:|SETUPSP,mc+ini-1,6,RND*2 -1:mc=mc mod totalm +1:dirm=dmet(mc) 
1116 goto 1120
...
4322 if ciclo= 100 then gosub  5000: GOTO 4310
4323 if ciclo< 200 then gosub 5500: GOTO 4310
...
4999' preparacion. solo en el ciclo inicial
5000 vx=1:sp=0:for i=20 to 29
5010 |SETUPSP,i,9,navemala3:|SETUPSP,i,0,&x1011:|LOCATESP,i,-30*(i-20),38
5020 |SETUPSP,i,5,5,vx: vx=-vx:
5030 gosub 500: gosub 750:|AUTOALL:|PRINTSPALL:|STARS
5040 next
5050 return

5499' logica de la primera orda lunar
5500 k=(k+1) mod 10: dirsp=27310+16*k: vx=peek (dirsp):|SETUPSP,k+19,6,-vx:
5510 return 

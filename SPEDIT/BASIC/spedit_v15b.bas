...
50 MODE 1
80 PEN 1:PRINT "    SPEDIT: Sprite Editor v15.0" 
90 PRINT "": INK 2,26:PEN 2 : INK 3,20
110 PRINT "   ---- CONTROLES EDICION ----"
120 PRINT "    1,2 : tinta -/+"
130 PRINT "    qaop, space : mueve y pinta"
140 PRINT "    ";chr$(242);chr$(243);":";:PRINT" desplaza medio byte" 
...
510 PEN 1:PRINT "qaop,sp:";:PEN blanco:PRINT"mueve,pinta"

520 PEN 1:PRINT "flechas:";:PEN blanco:PRINT" desplaza"
520 PEN 1:PRINT chr$(242);chr$(243);":";:PEN blanco:PRINT"desplaza ";(1+modo);" pix"
...
793 if asc(b$)=243 then gosub 6000:' desplazamiento derecha
794 if asc(b$)=242 then gosub 6200:' desplazamiento izq
...
6000'--- desplazamiento derecha ---
6010 LOCATE 1,25: PRINT "shifting..."
6020 FOR yf=0 TO altob-1 
6030 dirorigy = buffer + yf*anchob    
6040 dirdesty = &C000  + (INT (yf / 8) * 80) + (INT(yf MOD 8) * 2048) 
6041 lastb=0
6050 FOR xf=0 TO anchob-1
6060 dirorig = dirorigy+xf:dirdest=dirdesty+xf
6061 if modo=0 then 6094
6080 a=(PEEK(dirorig) AND &X11001100):'01
6090 b=(PEEK(dirorig) AND &x00110011):'23
6091 a=a/4:'new b
6092 lastb=lastb*4:'new a
6093 goto 6109
6094 a=(PEEK(dirorig) AND &X10101010):'0
6095 b=(PEEK(dirorig) AND &x01010101):'1
6096 a=a/2:'new b
6097 lastb=lastb*2 :'new a
6109 byte=lastb+a
6110 POKE dirdest, byte
6111 lastb=b
6120 next
6130 next
6140 goto 1300:'copia en buffer
6199 return

6200'--- desplazamiento izq ---
6210 LOCATE 1,25: PRINT "shifting..."
6220 FOR yf=0 TO altob-1  
6230 dirorigy = buffer + yf*anchob    
6240 dirdesty = &C000  + (INT (yf / 8) * 80) + (INT(yf MOD 8) * 2048) 
6241 lasta=0
6250 FOR xf=anchob-1 TO 0 step -1
6260 dirorig = dirorigy+xf:dirdest=dirdesty+xf
6261 if modo=0 then 6294
6280 a=(PEEK(dirorig) AND &X11001100):'01
6290 b=(PEEK(dirorig) AND &x00110011):'23
6291 b=b*4:'new a
6292 lasta=lasta/4:'new b
6293 goto 6309
6294 a=(PEEK(dirorig) AND &X10101010):'0
6295 b=(PEEK(dirorig) AND &x01010101):'1
6296 b=b*2:'new a
6297 lasta=lasta/2 :'new b
6309 byte=lasta+b
6310 POKE dirdest, byte
6311 lasta=a
6320 next
6330 next
6340 goto 1300:'copia en buffer
6399 return

...
mas		; actualizacion paso (decremento)
		ld HL,_ROUTE_TABLE
		ld b,0
		ld a, (ROU_SPID)
...
kk3		; estamos aqui porque el parametro es 1 byte (caso 255 y 254)
		cp 255
		;JR NZ, ROU_normal
		...
		JR Z, kk1; si es 0 es que es 255 y cambia estado
		; estamos aqui porque nos han pasado un 254 = cambio de secuencia

		ld bc, 7; posiciono hl para cambio de secuencia
		add hl,bc; hl= sprite dir sequence

kk1		ld (hl),a; actualiza la secuencia o el estado
kkk		ld hl, (ROU_dir); cargo en HL la direccion de esta ruta, que la tengo en ROU_dir

		jr ROU_sigseg
		;jr _UPDATE_SEGMENT


		; estamos aqui por que es 255. lo que viene a continuacion es un status
;		inc HL; apunta al estado que nos indican como parametro
;		ld (ROU_HL), HL; lo guardo
;		ld a, (HL); lee el parametro
;		pop HL; hl tiene la dir sprite (donde esta justo el estado)
;		ld (HL), a
...
;		push hl		
		;JR kk2
		;ld (HL), a
		;JP ROU_cont; si comento esto doy un paso mas
		;ld hl, (ROU_HL)
		;inc hl;
		
	; MEJORA v28. cambio de secuencia en mitad de ruta
	;----------------------------------------------------
		; si el numero de pasos es 254 entonces es que hay que cambiar de secuencia
;kk1			
		;cp 254
...
;		ld bc, 7; para cambio de imagen seria 9
;		add hl,bc
		;pop bc
;kk2		ld (hl),a; OJO A debe estar cargada con el parametro 
		;JP ROU_cont; si comento esto doy un paso mas

		;ahora debo actualizar el segmento del sprite. este ya esta hecho
;kkk		ld hl, (ROU_dir)
;		jr ROU_sigseg
...
;		ld (ROU_SEG),a
		...
		; y nos colocamos en el siguiente
;		ld hl, (ROU_HL)
;		inc hl; apunta al parametro void
;		inc hl; apunta al siguiente segmento de la ruta 
		;inc hl
